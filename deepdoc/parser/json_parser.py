...
#
"""json 解析器:把大 JSON/JSONL 按"序列化体积"切成多个保持结构的小块。

移植自官方 RAGFlow v0.24.0 deepdoc/parser/json_parser.py(逻辑 1:1)。
参考来源(官方注明):langchain_text_splitters 的 RecursiveJSONSplitter。

核心思路:
- 以 json.dumps 后的字符数近似衡量"块大小"(max_chunk_size 入参 ×2 为实际上限);
  递归遍历 dict,放得下当前块就写入(用路径还原嵌套结构),放不下且当前块已达
  min_chunk_size 就开新块。
- 列表默认转成 {"0": ..., "1": ...} 的索引字典再切(_list_to_dict_preprocessing),
  使数组元素也能按块拆分。
- 格式判定:整体能 json.loads 视为普通 JSON;否则采样前 10 行,≥80% 是合法
  JSON 行就按 JSONL 逐行解析,坏行直接跳过(永不抛错,解析失败返回 [])。
"""
...
class RAGFlowJsonParser:
    ...
    def __call__(self, binary):
        """入口:二进制内容;自动探测编码,按 JSONL/JSON 分流,返回字符串列表。"""
