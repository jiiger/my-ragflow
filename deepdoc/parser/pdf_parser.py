"""deepdoc/parser/pdf_parser.py — PDF 解析器全量, 对齐官方 v0.24.0(1859 行 1:1)。

三条链路:
- RAGFlowPdfParser(55-1771): 完整视觉管线 — __images__(pdfplumber 页渲染成图)
  → _layouts_rec(版面识别 onnx/ascend) → _table_transformer_job(表格结构识别)
  → _text_merge/_naive_vertical_merge/_concat_downward(文本合并/纵向合并)
  → _extract_table_figure(抽表格/插图, 输出位置坐标) → __call__(1521) 组装
  (text, position_tag) 段; extract_positions 解析 "@@页码,x0,y0,x1,y1##" 位置串;
  updown_concat_xgb 模型判断段落上下拼接; __html__/__markdown__/__pdf__ 出口
- PlainParser(1772): 纯文本路, 仅 pypdf.extract_text + outline 目录, 零模型依赖;
  crop/remove_tag 抛 NotImplementedError, 由 rag.nlp.tokenize_chunks 的 except 兜底走伪位置
- VisionParser(1806): RAGFlowPdfParser 子类 + vision_model, 每页渲染成图交给
  视觉 LLM 描述(vision_llm_chunk, rag/app/picture 链)

⚠️ 与官方差异: deepdoc.vision(OCR/LayoutRecognizer/TableStructureRecognizer)未移植,
顶层 import 改为 __init__ 内延迟 import — 实例化视觉管线时抛 ModuleNotFoundError
(提示清晰), PlainParser 纯文本路不受影响。模型权重 updown_concat_xgb.model 本地
rag/res/deepdoc 缺失时官方逻辑走 snapshot_download(HF), 保持原样。
"""
...
# ⚠️ deepdoc/vision 未移植: 顶层 try-import 保证模块可导入(docstring/F821 名字可见);
# RAGFlowPdfParser.__init__ 对 None 报清晰 ModuleNotFoundError; PlainParser 不触发
try:
    from deepdoc.vision import OCR, AscendLayoutRecognizer, LayoutRecognizer, Recognizer, TableStructureRecognizer  # noqa: F401
except ModuleNotFoundError:
    OCR = AscendLayoutRecognizer = LayoutRecognizer = Recognizer = TableStructureRecognizer = None  # type: ignore
    ...
    def __init__(self, **kwargs):
        ...
        if OCR is None:
            raise ModuleNotFoundError(
                "deepdoc.vision 未移植: 请先移植 deepdoc/vision(OCR/版面/表格识别链)再实例化视觉 PDF 解析器")
        self.ocr = OCR()
