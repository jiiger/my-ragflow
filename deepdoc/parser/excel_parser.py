...
#
"""excel 解析器:工作簿 → 行级 "表头: 值;..." 文本(另附 HTML/Markdown 输出)。

移植自官方 RAGFlow v0.24.0 deepdoc/parser/excel_parser.py(逻辑 1:1)。

核心思路:
- 加载兜底链:魔数识别(xlsx=PK.. / xls=\xd0\xcf..)→ 不是 Excel 就按 CSV
  读进 DataFrame 再转写 Workbook;openpyxl 打不开时依次退 pandas 默认引擎、
  calamine 引擎,全部转成统一 Workbook 再走后续逻辑。
- 大表防护:max_row 超过 10000 时二分 + 局部扫描找真实最后一行,防止
  全表误报的 max_row 拖垮 iter_rows。
- __call__:首行当表头,每行拼成 "表头: 值;..." 的句子;sheet 名不含
  "sheet" 时在行尾追加 " ——sheet名"(保留业务表名信息)。
- html():表格转 <table> 分块(chunk_rows 行一块);markdown():整表转 md。
"""
...
class RAGFlowExcelParser:
    ...
    def _load_excel_to_workbook(file_like_object):
        # ⚠️ 官方疑似 bug(1:1 保持):这里只把 bytes 包装成 BytesIO,str 路径
        # 会直接撞上下面的 seek(0) 抛 AttributeError。官方生产调用方
        # (rag/app/naive.py、one.py、flow/parser)全部传二进制,从不传路径,
        # 故该分支实际走不到;如需路径输入请先自行读成 bytes。
    ...
    def _get_actual_row_count(ws):
        """真实行数:max_row ≤10000 直接信;超限时二分找最后一个有数据的行。

        背景:某些工具生成的 xlsx 会把 max_row 报得虚大(如百万空行),
        直接 iter_rows 会卡死。二分定位后还会向后多扫 500 行兜底。
        """
        max_row = ws.max_row
        if not max_row:
            return 0
    ...
    def __call__(self, fnm):
        """入口:路径(str)或二进制;返回每数据行一条 "表头: 值;..." 文本。"""
        file_like_object = BytesIO(fnm) if not isinstance(fnm, str) else fnm
        wb = RAGFlowExcelParser._load_excel_to_workbook(file_like_object)
    ...
    def row_number(fnm, binary):
        """预估行数(task_executor 用它做进度百分比):xls 走 Workbook 统计,csv/txt 数换行。"""
