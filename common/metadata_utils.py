...
"""元数据过滤工具(与官方 common/metadata_utils.py 1:1 移植, 2026-09-02)。

convert_conditions: 把前端条件(comparison_operator/name/value)转成
    meta_filter 需要的 {op/key/value} 结构(同时做符号归一化 is→= 等);
meta_filter: 按一组过滤条件对 {key: {value: [doc_id]}} 元数据索引做
    and/or 组合过滤, 支持 =/≠/></≥/≤/contains/in/start/end/empty 等运算,
    并对 YYYY-MM-DD 日期做字符串级比较、非日期值做 ast.literal_eval 再比较;
apply_meta_data_filter: 三类过滤方式编排(auto 用 LLM 生成条件 → 调用
    gen_meta_filter, 该 import 放函数内避免循环; semi_auto 只取选定 key;
    manual 直接用前端传入条件), 返回命中的 doc_id 列表;
dedupe_list / update_metadata_to: 去重与元数据增量合并;
metadata_schema / turn2jsonschema: 元数据(描述列表)↔ JSON Schema 互相转换,
    供知识库元数据 schema 校验与前端表单渲染用。
"""
