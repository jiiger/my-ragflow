...
"""ES 连接基类:索引/文档层通用操作(换引擎时最多只改这一层)。

移植自官方 v0.24.0 common/doc_store/es_conn_base.py(332 行, 逻辑 1:1)。
- 启动即读 conf/mapping.json 作为索引 mapping 模板(分片 2/无副本/脚本化
  相似度), create_idx 按"模板 + 向量维度"建 chunk 索引与元数据索引
- 提供 health/get/insert/update/delete/search 骨架, 检索语义在
  rag/utils/es_conn.py 的 ESConnection 里按混合检索重写
- infinity_conn_base/ob_conn_base 官方同层实现, 学习版未移植(选型 ES)"""
