...
#
"""向量库抽象层:全库统一的文档存储接口与查询表达式数据类。

移植自官方 v0.24.0 common/doc_store/doc_store_base.py(270 行, 逻辑 1:1)。
docStoreConn(settings 里装配的 DocStoreConnection 单例)的所有操作都经这一层抽象:
- DocStoreConnection: 索引(chunk 索引/元数据索引)与文档 CRUD 的统一接口
- 表达式数据类: SparseVector(稀疏向量)、MatchTextExpr(关键词匹配)、
  MatchDenseExpr(稠密向量)、MatchSparseExpr(稀疏向量)、MatchTensorExpr、
  FusionExpr(RRF/加权融合)、OrderByExpr(排序)——ES/Infinity/OceanBase
  各自的落地实现把表达式翻译成自家查询语法, 上层(检索/入库)只认抽象。"""
