"""dialog_service.py — 问答助手(dialog)服务层核心, 对齐官方 v0.24.0(1203 行)。

学习要点:
- DialogService: dialog 表 CRUD。get_by_tenant_ids 用 JOIN User 取 tenant 昵称/头像,
  关键词过滤走 fn.LOWER(name).contains(大小写不敏感), parser_id 可筛助手类型
- async_chat: 聊天主流程(多轮问答 + RAG), 按阶段拆解:
  1) 前置: langfuse 埋点(可选) → get_models 装配 embd/chat/rerank/tts 模型
  2) 问题润色: full_question 追问补全 / cross_languages 跨语言 / keyword_extraction
     关键词抽取; meta_data_filter 元数据过滤(apply_meta_data_filter)
  3) 检索: reasoning 分支走 DeepResearcher 深度研究(队列流式输出过程);
     常规分支 Dealer.retrieval(embd + rerank + rank_feature=label_question 问题分类
     + toc_enhance 目录增强 + retrieval_by_children 父子块召回); tavily 外网检索
     可选; use_kg 图检索(settings.kg_retriever)
  4) 拼 prompt: kb_prompt 组织知识段, citation_prompt 引用指令, message_fit_in
     按 max_tokens*0.95 裁剪历史消息
  5) 生成: async_chat_streamly_delta 流式, _stream_with_think_delta 拆出
     " thinking"/" response" 思考标记, decorate_answer 把 [ID:n] 引用映射到
     doc_id → 组 refs(引用文档 doc_aggs), 附分阶段耗时与 token 统计
- use_sql: 表格知识库 SQL 问答。按 ES/Infinity/OceanBase 引擎拼 DBA prompt →
  LLM 写 SQL → settings.retriever.sql_retrieval 执行 → Markdown 表格组装 +
  来源列 ##n$$ 引用标记; 失败带错误信息重试一次; 聚合查询补查 doc_id/docnm_kwd
- async_ask: 无会话历史的单发问答(检索 + 摘要 prompt), gen_mindmap: 检索
  结果喂 MindMapExtractor 生成思维导图
- 引用修复: repair_bad_citation_formats 处理 (ID:12)/[ID:12]/【ID:12】/ref12
  等 LLM 常输出的坏引用格式

与官方差异: 仅将 5 个未移植模块改为函数内延迟 import + ⚠️ 注释
(DocMetadataService/metadata_utils/advanced_rag/tag/tavily_conn), 其余代码 1:1。
"""
