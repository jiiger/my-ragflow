...
"""RAG 评测服务(与官方 api/db/services/evaluation_service.py 1:1 移植, 2026-09-02)。

对某对话配置做自动化评测: 拿评测集的测试题跑真实 RAG 管线(async_chat),
算检索/生成指标并给调参建议。

数据模型(均已在 db_models): EvaluationDataset / EvaluationCase /
EvaluationRun / EvaluationResult。

- 数据集管理: create/list/update/delete_dataset;
- 测试用例: add/get/delete/import_test_cases(bulk_create 批量导入);
- 评测执行: start_evaluation(建 run + 存 dialog 配置快照 → 同步跑
  _execute_evaluation → 逐条 _evaluate_single_case → _compute_summary_metrics),
  实时调用 dialog_service.async_chat 把 async 生成器桥回同步线程消费;
- 指标: 检索侧 precision/recall/f1/hit_rate/mrr(用相关块 ground truth),
  生成侧 answer_length/has_answer; LLM-as-judge 高级指标留 TODO;
- 结果与建议: get_run_results / get_recommendations(按 avg 指标给调参建议)。
"""
