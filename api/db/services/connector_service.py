...
"""外部数据源连接器服务(与官方 api/db/services/connector_service.py 1:1 移植, 2026-09-02)。

管理 RAGFlow 轮询外部数据源(Connector)与知识库的绑定、同步任务调度和日志。

- ConnectorService: 连接器对多个知识库的同步任务恢复/列表/重建;
- SyncLogsService: 同步任务日志 CRUD + 调度(schedule, 超 100 条清理旧日志,
  已有 SCHEDULE 任务时告警)、start/done 状态流转、increase_docs 计数与
  拉取区间累积、duplicate_and_parse 把抓取到的文档转成 FileService 上传簇;
- Connector2KbService: 连接器↔知识库绑定关系, link_connectors 做差量
  (新增/更新 auto_parse/解除绑定取消进行中任务)、list_connectors。

依赖差异: 官方把 DocMetadataService 从 document_service 再导出后在此顶层
import; 学习版 document_service 用延迟 import, 故这里直接从其源模块
doc_metadata_service 导入(DocMetadataService 定义处), 逻辑一致。
其余依赖 (anthropic.BaseModel/InputType/TaskStatus/CommonService/
FileService) 均已就位。
"""
