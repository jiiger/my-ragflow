"""search_service.py — 检索会话(Search)服务, 对齐官方 v0.24.0(121 行)。

学习要点:
- Search 表存一次"检索任务"的配置(search_config)与展示信息(名字/描述/头像),
  与聊天(conversation)区分: 检索只召回不生成答案, 供 conversation_app 的
  search 模式与侧边检索用
- save 补 create_time/create_date 双字段(时间戳+日期); get_detail/get_by_tenant_ids
  JOIN User 取 tenant 昵称/头像, 权限过滤 created_by/tenant_id;
  accessible4deletion 用来校验删除权(仅创建者本人、VALID 状态)
"""
