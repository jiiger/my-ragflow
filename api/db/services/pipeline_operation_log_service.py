...
#
"""管道操作日志(PipelineOperationLog)服务 — 移植自官方 v0.24.0。

记录/查询知识库管道(pipeline)运行日志。前端知识库详情页
list_pipeline_logs / list_pipeline_dataset_logs 依赖这里的
get_file_logs_by_kb_id / get_dataset_logs_by_kb_id(只读, 不触 agent 画布)。

create/record_pipeline_operation 的 pipeline_id 分支会用 UserCanvasService
(画布未移植) → 延迟 import + 缺失时降级用 document.parser_id 作标题。
来源: /home/yang/code/ragflow/api/db/services/pipeline_operation_log_service.py (tag v0.24.0)
"""
import json
import logging
import os
...
class PipelineOperationLogService(CommonService):
    ...
    def create(cls, document_id, pipeline_id, task_type, fake_document_ids=[], dsl: str = "{}"):
        ...
        if pipeline_id:
            # ⚠️ 延迟 import: 画布(canvas_service)未移植 → 缺失时降级用 parser_id 作标题
            try:
                from api.db.services.canvas_service import UserCanvasService  # ⚠️ 未移植, 延迟 import
                ...
                logging.warning("canvas_service not ported, fallback to document.parser_id for pipeline log title")
                tenant_id = document.tenant_id if hasattr(document, "tenant_id") else None
