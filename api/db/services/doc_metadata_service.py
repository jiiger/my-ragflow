...
"""文档元数据服务(与官方 api/db/services/doc_metadata_service.py 1:1 移植, 2026-09-02)。

文档级元数据的唯一事实来源, 存于 ES/Infinity 的每租户专属索引
ragflow_doc_meta_{tenant_id}(官方已移除 MySQL 的 meta_fields 列)。

主要方法:
- insert / update / get / delete_document_metadata: 单文档元数据的
  增删改查(ES 走局部更新, Infinity 因不支持回退到 delete+insert);
- _drop_empty_metadata_table: 删除文档后若表空则顺手 DROP(避免空索引累积);
- get_meta_by_kbs(legacy, 列表值当整体字符串键) / get_flatted_meta_by_kbs
  (展开列表为单个键): 两种聚合语义, 供 meta_filter 过滤用;
- get_metadata_for_documents / get_metadata_summary: 按文档取元数据 /
  汇总字段类型与取值分布(前端筛选器渲染用);
- batch_update_metadata: 批量 update/delete 操作(支持 match 精确替换列表项),
  无元数据行的文档走 insert 兜底;
- _split_combined_values: 把 LLM 抽出的"关羽、孙权、张辽"混合值按常见分隔符拆分。

依赖: settings.docStoreConn(ESConnection, 提供 create_doc_meta_idx/
index_exist/insert/search/get/delete/delete_idx/es)、OrderByExpr、
metadata_utils.dedupe_list —— 均已就位。方法均双层装饰(注意 @classmethod
在 @DB.connection_context() 之上, 见 SKILL peewee 坑)。
"""
