...
async def create():
    """创建知识库: 校验名称后 create_with_name 生成模型/索引, 再落库并返回 kb_id。"""
...
async def update():
    """更新知识库信息: 名称查重/权限校验/同步重命名文件夹/联动 pagernk 与 connectors。"""
    req = await get_request_json()
    ...
    try:
        ...
        from api.db.services.connector_service import Connector2KbService  # ⚠️ connector_service 未移植, 延迟 import
...
async def update_metadata_setting():
    """更新知识库元数据设置: 把 metadata 配置写回 parser_config。"""
    req = await get_request_json()
    e, kb = KnowledgebaseService.get_by_id(req["kb_id"])
    if not e:
        return get_data_error_result(
...
@login_required
def detail():
    """知识库详情: 权限校验后返回基本信息/文档总大小/连通的 connector/元数据 schema。"""
    ...
    try:
        ...
        if kb["parser_config"].get("metadata"):
            from common.metadata_utils import turn2jsonschema  # ⚠️ metadata_utils 未移植, 延迟 import
...
@login_required
async def list_kbs():
    """知识库列表: 支持关键词/分页/排序/parser 过滤, 可选按 owner_ids 过滤。"""
    args = request.args
...
async def rm():
    """删除知识库: 级联删文档/文件/索引/存储桶, 再删库记录(线程池同步执行)。"""
    req = await get_request_json()
    uid = current_user.id
...
async def knowledge_graph(kb_id):
    """取知识图谱: 检索图中带 knowledge_graph_kwd 的文档, 反序列化出 graph/mind_map。"""
    if not KnowledgebaseService.accessible(kb_id, current_user.id):
        return get_json_result(
            data=False,
            message="No authorization.",
...
@login_required
def get_meta():
    """取知识库元数据 schema(拍平后的 metadata 字段列表)。"""
    kb_ids = request.args.get("kb_ids", "").split(",")
    for kb_id in kb_ids:
        if not KnowledgebaseService.accessible(kb_id, current_user.id):
            return get_json_result(
    ...
    from api.db.services.doc_metadata_service import DocMetadataService  # ⚠️ doc_metadata_service 未移植, 延迟 import
...
@login_required
def get_basic_info():
    """知识库基础统计: 文档数/字符数/解析进度等(kg 为 0 时跳过图字段)。"""
...
async def list_pipeline_logs():
    ...
    try:
        from api.db.services.pipeline_operation_log_service import PipelineOperationLogService  # ⚠️ 未移植, 延迟 import
...
async def run_graphrag():
    """启动 GraphRAG 任务: 校验 KB 与文档后, 把任务塞进任务队列并记录 task_id。"""
    req = await get_request_json()
    ...
    if task_id:
        from api.db.services.task_service import TaskService  # ⚠️ task_service 未移植, 延迟 import
    ...
    from api.db.services.task_service import GRAPH_RAPTOR_FAKE_DOC_ID  # ⚠️ task_service 未移植, 延迟 import
...
async def run_raptor():
    """启动 RAPTOR 任务: 递归摘要聚类, 逻辑同 run_graphrag。"""
    req = await get_request_json()
...
async def run_mindmap():
    """启动 MindMap 任务: 文档关系抽取成思维导图, 逻辑同 run_graphrag。"""
    req = await get_request_json()
...
@login_required
def delete_kb_task():
    """解绑知识库的管道任务(graphrag/raptor/mindmap): 发取消信号 + 清 task_id 与完成时间。"""
    kb_id = request.args.get("kb_id", "")
    if not kb_id:
        return get_error_data_result(message="Lack of \"KB ID\"")
    ok, kb = KnowledgebaseService.get_by_id(kb_id)
...
async def check_embedding():
    """embedding 模型切换自检: 采样 KB 里已有向量的 chunk, 算新旧向量的余弦相似度, 均值 >0.9 才放行。"""
    ...
    def sample_random_chunks_with_vectors(
        ...
        """在 KB 文档索引里随机抽 n 个带向量的 chunk, 连同向量一并返回。"""
        from common.doc_store.doc_store_base import OrderByExpr  # ⚠️ common/doc_store 未移植, 延迟 import
    ...
    summary = {
        ...
        # ⚠️ 官方原样: 采样为空时 mode 未定义会 NameError
        "match_mode": mode,
    }
