...
"""Langfuse 可观测性配置路由(与官方 api/apps/langfuse_app.py 1:1 移植, 2026-09-02)。

管理当前租户的 Langfuse 密钥(public_key/secret_key/host):

- POST/PUT /api_key: 校验 keys 有效(auth_check)后保存或更新;
- GET /api_key: 读出已存 keys, 校验有效后附上 project_id/project_name;
- DELETE /api_key: 删除租户 keys。

依赖 langfuse(langfuse-python 已装) + TenantLangfuseService 均在位。
"""
