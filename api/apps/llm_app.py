...
#
"""
api/apps/llm_app.py — LLM 模型管理路由(厂商/密钥/模型注册与启停)。

核心流程: 厂商白名单 + 模型注册表(rag/llm 工厂 dict) + TenantLLM 租户级配置;
set_api_key / add_llm 都会用真实模型调用做"打点验证", 失败则拒绝写入。
来源: 官方 api/apps/llm_app.py @ v0.24.0
"""
import asyncio
import json
import logging
...
@login_required
def factories():
    """LLM 厂商列表: 每个厂商附带其可用的模型类型(从全局注册表的有效模型算)。"""
    try:
...
async def set_api_key():
    """设置厂商 API Key: 用 embedding/chat/rerank 三类模型真实调用验证, 通过才写 TenantLLM。
    ⚠️ LLM_TIMEOUT_SECONDS 同时决定 openai 客户端的 connect 超时(连不可达端点会挂很久);
    调试时设小(如 5)并用 shell timeout 兜底。
    """
...
async def add_llm():
    """新增模型: 按厂商拼装特殊鉴权字段(ark/腾讯云/Bedrock 等), 按类型验证后落 TenantLLM。
    llm_name 的 ___厂商 后缀用于区分不同厂商的同名模型(list_app 里按此去重)。
    """
    req = await get_request_json()
    factory = req["llm_factory"]
    ...
    elif factory == "OpenRouter" or factory == "MinerU" or factory == "PaddleOCR":
...
@login_required
def my_llms():
    """当前租户已配置的模型列表: 按厂商聚合, 含用量/状态; include_details 时带厂商标签。"""
    try:
...
@login_required
async def list_app():
    """模型选择器列表: 全局注册表 + 租户自配模型合流, 按是否可用打 available 标记。"""
