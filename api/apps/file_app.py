...
#
"""
api/apps/file_app.py — 文件与文件夹管理路由(个人文件盘, 含知识库文件两种来源)。

覆盖上传(支持带相对路径自动建多层文件夹)/建目录/列表/文件夹链查询/删除(递归)/
重命名/下载/移动; 删除与移动都在线程池里同步执行, 权限统一走
check_file_team_permission 的团队级校验。
来源: 官方 api/apps/file_app.py @ v0.24.0
"""
import logging
import os
import pathlib
...
async def upload():
    """上传文件(可带相对路径自动建目录): 根目录兜底 + 数量上限 + 存储后建档。

    ⚠️ settings.STORAGE_IMPL 为 None 占位(对象存储未接入), 存储读写调用暂不可用。
    """
...
async def create():
    """新建文件夹(FOLDER)或虚拟文件(VIRTUAL)到指定 parent_id 下。"""
...
@login_required
def list_files():
    """目录文件列表: 关键词/分页/排序; 首次访问根目录时补建知识库文件夹。"""
...
async def rm():
    """删除文件/文件夹: 文件夹递归删全部子项, 文件连带删存储对象与关联文档。

    ⚠️ 存储层对象删除依赖 settings.STORAGE_IMPL(占位 None); 知识库来源文件跳过。
    """
    req = await get_request_json()
...
async def get(file_id):
    """取文件内容流: 文件盘存储优先, 回退到文档存储地址, 按扩展名设 Content-Type。
    ⚠️ 存储读取依赖 settings.STORAGE_IMPL(占位 None)。
    """
    try:
        e, file = FileService.get_by_id(file_id)
        if not e:
...
async def move():
    """移动文件/文件夹到目标目录: 同名文件夹合并, 存储层对象同步迁移。
    ⚠️ 存储层 move/obj_exist 依赖 settings.STORAGE_IMPL(占位 None)。
    """
    req = await get_request_json()
    try:
