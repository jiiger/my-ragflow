"""dialog_app.py — 问答助手(dialog)路由层, 对齐官方 v0.24.0(248 行, 1:1 无裁剪)。

接口: /v1/dialog/{set,get,list,next,rm}
- set: 创建/更新助手。创建时查重名(casefold 后 duplicate_name 去重);
  校验 {knowledge} 占位符与 parameters 一致性; 多知识库必须同 embedding 模型;
  更新时要求有 kb_ids 或 tavily, 否则报错提示删 {knowledge}
- get/list: 助手详情/列表, kb_ids 经 get_kb_names 展开成 [{id, name}]
- next: 分页列表(tenant 聚合), owner_ids 指定则按租户过滤
- rm: 软删(仅租户成员有权限, status→INVALID)
"""
