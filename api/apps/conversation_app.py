"""conversation_app.py — 会话对话路由层, 对齐官方 v0.24.0(478 行, 1:1 无裁剪)。

接口: /v1/conversation/{set,get,rm,list,completion,sequence2txt,tts,delete_msg,
      thumbup,ask,mindmap,related_questions} + /getsse/<dialog_id>
- set/get/list/rm: 会话 CRUD(会话挂在 dialog 下, 权限校验收租户成员)
- completion: 核心问答接口。messages 过滤 system/开头 assistant → async_chat
  流式 → 每帧 structure_answer 落库 → SSE; 可指定 llm_id(内嵌模式 is_embedded
  时不落库); 非流式走单帧断点
- ask: 单发问答(async_ask, 支持 search_id 复用检索配置); mindmap: 思维导图
  (gen_mindmap); related_questions: LLM 生成相关问题(load_prompt 模板)
- sequence2txt: 语音转写(ASR, 白名单后缀 + 临时文件); tts: 文本转语音流
  (按标点切句逐段合成); getsse: API token(APIToken.beta)换取助手信息
- delete_msg/thumbup: 消息撤销(连同 reference)与点赞/反馈
"""
