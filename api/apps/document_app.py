...
async def upload():
    """上传文件到知识库: 校验名称/权限后走 FileService.upload_document 落库。"""
...
async def web_crawl():
    """爬取网页转 PDF 入库: 校验 url 后渲染 + 建档挂到知识库文件夹。"""
    form = await request.form
    kb_id = form.get("kb_id")
    if not kb_id:
        return get_json_result(data=False, message="Lack of \"KB ID\"", code=RetCode.ARGUMENT_ERROR)
    ...
    if not e:
        raise LookupError("Can't find this dataset!")
    # ⚠️ 官方原样: 此处权限判断与 upload() 相反(通过校验反而拒绝), 疑似官方笔误, 保持 1:1
...
async def create():
    """建空文档(VIRTUAL 类型): 挂在知识库文件夹下, 不落存储。"""
...
@login_required
async def list_docs():
    """文档列表: 关键词/状态/类型/后缀/时间/元数据过滤 + 分页排序。"""
    ...
    if metadata_condition or metadata:
        from api.db.services.doc_metadata_service import DocMetadataService  # ⚠️ 元数据链未移植, 延迟 import
        from common.metadata_utils import meta_filter, convert_conditions  # ⚠️ metadata_utils 未移植, 延迟 import
...
async def get_filter():
    """文档筛选器: 返回可用的 run_status/types/suffix 组合候选。"""
    req = await get_request_json()
...
async def metadata_update():
    """批量更新/删除文档元数据(updates/deletes 两条变更列表)。"""
    req = await get_request_json()
    kb_id = req.get("kb_id")
...
async def update_metadata_setting():
    """更新单文档的元数据设置(写回 parser_config.metadata)。"""
    req = await get_request_json()
...
async def change_status():
    """启停文档: 状态 0/1 同步到数据库和 docStore(有 chunk 时)。"""
    req = await get_request_json()
    doc_ids = req.get("doc_ids", [])
...
async def rm():
    """删除文档: 权限校验后由 FileService.delete_docs 连带删文件与索引。"""
    req = await get_request_json()
    doc_ids = req["doc_id"]
...
async def run():
    """启停文档解析任务: RUNNING/CANCEL 状态机 + 可选 delete 清理旧索引(线程池同步)。"""
    req = await get_request_json()
    uid = current_user.id
    try:
        def _run_sync():
            ...
            for id in req["doc_ids"]:
                ...
                if str(req["run"]) == TaskStatus.CANCEL.value:
                    from api.db.services.task_service import TaskService, cancel_all_task_of  # ⚠️ task_service 未移植, 延迟 import
...
async def rename():
    """文档重命名: 后缀不可变, 同步改文件表与 docStore 标题索引(线程池同步)。"""
    req = await get_request_json()
    uid = current_user.id
    try:
        def _rename_sync():
            ...
            from rag.nlp import rag_tokenizer  # ⚠️ rag_tokenizer 分词未移植, 延迟 import
...
async def change_parser():
    """更换解析器/流水线: 重置进度并回滚已入库的 chunk 与图片。"""
    req = await get_request_json()
    if not DocumentService.accessible(req["doc_id"], current_user.id):
        return get_json_result(data=False, message="No authorization.", code=RetCode.AUTHENTICATION_ERROR)
...
async def get_image(image_id):
    """取 chunk 内嵌图片(kb_id-文件名 拆分定位存储桶与对象名)。"""
    try:
        arr = image_id.split("-")
        if len(arr) != 2:
...
async def upload_and_parse():
    """对话内上传并解析(不走知识库): doc_upload_and_parse 返回 doc_id 列表。"""
    files = await request.files
    if "file" not in files:
        return get_json_result(data=False, message="No file part!", code=RetCode.ARGUMENT_ERROR)
...
@login_required
async def parse():
    """解析文件/网页为文本: url 分支用 seleniumwire 抓取, 文件分支走 FileService.parse_docs。"""
    req = await get_request_json()
    url = req.get("url", "")
    if url:
        if not is_valid_url(url):
        ...
        # ⚠️ 官方函数内 import seleniumwire(重依赖/需浏览器), 未装时该 url 分支报 ImportError
        ...
        if len(res_headers) > 1:
            from deepdoc.parser.html_parser import RAGFlowHtmlParser  # ⚠️ deepdoc 未移植, 延迟 import
...
async def set_meta():
    """设置文档元数据: meta 必须是 JSON map, 值只允许 str/int/float 或它们的列表。"""
    req = await get_request_json()
    if not DocumentService.accessible(req["doc_id"], current_user.id):
        return get_json_result(data=False, message="No authorization.", code=RetCode.AUTHENTICATION_ERROR)
    try:
