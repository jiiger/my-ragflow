"""chunk_app.py — 文档块(chunk)管理路由层, 对齐官方 v0.24.0(497 行)。

chunk = 文档解析入库后的最小检索单元(ES 文档), 前端知识库"文档→块"管理全靠它:
- list: 按 doc_id 分页列块(带 keywords 内容高亮); get: 单块详情(剥掉向量/分词字段)
- set: 改块 — 重分词 → 重向量化(0.1×标题向量+0.9×内容向量) → 更新 ES;
  QA 类型文档按问答对重组(rmPrefix/beAdoc); 可顺带换块图片
- create: 手工建块 — xxhash64(内容+doc_id) 幂等 id(重复提交不重复) →
  embedding → 写入 ES → 递增文档 chunk 计数(token 数 c 一并记账)
- rm: 批量删块(ES 删除 + 计数扣减 + 对象存储图片清理); switch: 批量启停
  available_int(停用的块不参与检索)
- retrieval_test: 检索测试台(知识库调参用) — 完整链路: 元数据过滤(auto 需 chat
  LLM) / 跨语言 / 关键词抽取 / embd+rerank 检索 / KG 图检索 / 子块召回,
  返回 ranks + 问题分类 labels
- knowledge_graph: 取文档 GraphRAG 知识图谱/思维导图(ES 按 graph/mind_map
  标记过滤, 思维导图节点重名去重)

与官方差异: 4 个未移植模块延迟 import 到使用点 — rag.app.qa(/set QA 分支)、
rag.app.tag(/retrieval_test)、doc_metadata_service+metadata_utils
(/retrieval_test 元数据过滤分支), 其余 1:1。
"""
    ...
    try:
        def _set_sync():
            ...
            if doc.parser_id == ParserType.QA:
                # ⚠️ 延迟 import: rag.app.qa 未移植(仅 QA 类型文档改块用)
...
async def retrieval_test():
    ...
    async def _retrieval():
        ...
        # ⚠️ 延迟 import: rag.app.tag 未移植(问题分类标签)
