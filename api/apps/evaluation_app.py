...
"""RAG 评测 API(与官方 api/apps/evaluation_app.py 1:1 移植, 2026-09-02)。

面向评测面板的 REST 接口, 全部委托 EvaluationService:

- 数据集: /dataset/create|list、/dataset/<id> 的 GET/PUT/DELETE;
- 用例: /dataset/<id>/case/add|import、/dataset/<id>/cases、
  /case/<id>(DELETE);
- 执行: /run/start、/run/<id>(GET|DELETE)、/run/<id>/results、
  /run/list、/run/<id>/recommendations、/run/<id>/export;
- 实时: /evaluate_single(官方为 TODO 空实现, 保留原样);
- 对比: /compare(TODO); 列表/删除/导出也有官方 TODO, 均保留。

官方多处接口是桩(TODO: Implement ...), 学习版 1:1 保留, 不改动。
"""
