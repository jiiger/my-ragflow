"""search_app.py — 检索会话(Search App)路由层, 对齐官方 v0.24.0(187 行, 1:1 无裁剪)。

接口: /v1/search/{create,update,detail,list,rm}
- create: 建检索任务。名字校验/去重(duplicate_name 按 tenant 查重),
  补 id/tenant_id/created_by 后 DB.atomic() 写入 Search 表
- update: 改检索配置。被 not_allowed_parameters 挡掉 id/created_by/时间戳;
  accessible4deletion 校验仅创建者可改; search_config 做浅合并
  ({**旧配置, **新配置}), 便于客户端只传要改的字段
- detail: 详情(JOIN User 取 tenant 头像), 先按租户成员校验权限
- list: 分页列表(owner_ids 指定租户则内存过滤), 同 dialog/next 模式
- rm: accessible4deletion 校验后按 id 硬删
"""
