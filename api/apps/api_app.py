...
"""API Token 路由(与官方 api/apps/api_app.py 1:1 移植, 2026-09-02)。

为某对话/画布签发与统计 API 访问凭证:

- /new_token: 为当前用户的第一个租户生成 token(可绑 dialog_id, 或
  canvas_id 时 source=agent), 存 APIToken;
- /token_list: 查某 dialog/canvas 下所有 token;
- /rm: 按 token 列表 + 租户删凭证;
- /stats: 按日期区间统计该租户会话的 pv/uv/speed/tokens/round/thumb_up,
  供前端用量可视化(速度 = tokens/(duration+0.1) 防除零, tokens 转千单位)。

依赖 api_service(APITokenService/API4ConversationService)/UserTenantService/
api_utils, 均在位。裸 manager 由 register_page 注入。
"""
