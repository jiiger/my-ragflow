...
"""检索器(与官方 rag/nlp/search.py 1:1 移植, 仅加学习注释)。

Dealer 是 RAG 检索核心, 被 settings.retriever 实例化(单个 docStoreConn 连接):
- index_name(uid): 租户 ES 索引名 "ragflow_{uid}"
- search(): 混合检索 —— 全文 MatchTextExpr + 向量 MatchDenseExpr 经
  FusionExpr(weighted_sum, 全文 0.05 / 向量 0.95)融合; 无结果时降级
  (降低 min_match / 放宽 similarity); 返回 SearchResult(total/ids/query_vector/
  field/highlight/aggregation/keywords)
- get_vector(): 问句向量化 -> q_{dim}_vec 列 -> cosine MatchDenseExpr
- retrieval(): 业务层封装 —— search 拿候选(RERANK_LIMIT ≈ ceil(64/ps)*ps)
  → 精排(rerank_by_model 用 rerank 模型, rerank 本地混合相似度) →
  阈值过滤(默认 0.2) → 分页切片 → 组装 chunk dict(含 similarity/
  vector_similarity/term_similarity) → doc_aggs 按文档聚合
- rerank(): 本地精排, hybrid_similarity = 向量余弦*0.7 + 词重叠*0.3,
  词特征里 title×2 / important×5 / question×6 刻意放大, 再加 tag/pagerank
- insert_citations(): 把答案切句, 每句与 chunk 向量算相似度, 相似片段
  标 [ID:n](问答引用溯源)
- chunk_list / sql_retrieval / tag_content / tag_query / all_tags:
  块列表、ES SQL、tag 特征打分(0.1*(c+1)/(total+S) 归一)
- retrieval_by_toc(): 目录增强 —— 先命中最高相似文档, LLM 从 toc 选块补分
- retrieval_by_children(): 父子块 —— 子块命中时把父块拉回, 内容拼接,
  相似度取子块均值

依赖: query(FulltextQueryer)、term_weight、synonym、rag_tokenizer、
doc_store_base(表达式+DocStoreConnection 抽象)、string_utils、float_utils。

官方文件: rag/nlp/search.py@v0.24.0
"""
import json
import logging
import re
