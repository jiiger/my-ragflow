...
"""词项权重计算(与官方 rag/nlp/term_weight.py 1:1 移植, 仅加学习注释)。

Dealer 是查询/文档侧的词典加权器, 供 FulltextQueryer(rag/nlp/query.py) 使用:
- pretoken: 分词 + 去停用词/纯数字 + 标点归一为 "#"
- token_merge: 单字/短词连续片断合并(中文"多 工位"式粘连)
- split: 英文词组拼接(两段连续英文且非函数名则合并)
- weights: 综合 4 个信号算词权重并归一:
    ① ner(t): 命名实体权重(数字2/机构地名学校股票3/代词0.01/常规1)
    ② postag(t): rag_tokenizer.tag 词性(代词连词副词0.3/专名3/名词2)
    ③ freq(t): 词频 IDF(fine_grained 分解兜底, 英文生词按 300)
    ④ df(t): 文档频率 IDF(term.freq 词典, 缺失 fallback 3)
  weights 最终 = (0.3*idf1 + 0.7*idf2) * ner * postag, 归一后输出。
⚠️ 资源文件 rag/res/ner.json(实体词典)——学习版需随仓库带上, 缺失仅 warning。

官方文件: rag/nlp/term_weight.py@v0.24.0
"""
