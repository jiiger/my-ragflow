...
"""同义词查询(与官方 rag/nlp/synonym.py 1:1 移植, 仅加学习注释)。

Dealer 给查询词找同义词, 供 FulltextQueryer 扩展 Lucene 查询:
1. 优先查自定义词典 rag/res/synonym.json(小写键)
2. 纯英文字母词 fallback 到 WordNet(wordnet.synsets, 去掉原词自身)
3. redis 插件(kw: "kevin_synonyms")每小时热更新, 无 redis 则禁用
⚠️ 资源文件 rag/res/synonym.json——学习版需随仓库带上, 缺失仅 warning。
nltk 语料(wordnet)首次使用前需下载, 见 SKILL pitfalls。

官方文件: rag/nlp/synonym.py@v0.24.0
"""
