...
"""全文查询器(与官方 rag/nlp/query.py 1:1 移植, 仅加学习注释)。

FulltextQueryer(QueryBase 子类) 把自然语言问句编译成 ES Lucene 查询串:
- query_fields 带 boost: title_tks^10 / important_kwd^30 / question_tks^20 /
  content_ltks^2(标题、关键词权重远高于正文)
- question(): 预处理(简体/全半角/中英空格)-> 英文路径(tokenize+词权重+
  同义词+相邻 bigram 短语)或中文路径(term_weight.split 切词 + fine_grained
  细粒度扩展 + 同义词 + 编辑距离模糊匹配) -> MatchTextExpr
  min_match 控制宽松度(命中 30% 即算匹配, 无结果时调用方降为 0.1)
- hybrid_similarity(): 向量余弦(cosine_similarity)*vtweight + 词重叠
  (token_similarity, 含相邻词 bigram)*tkweight —— Dealer.rerank 用
- token_similarity(): 两边 token 各自算权重 dict(单词 0.4 + 邻词 0.6),
  再 similarity() 算重叠占比(命中词权重和 / 查询词权重和)
- paragraph(): 段落标签聚合查询(tag_content 用, 权重 topN 关键词转查询)

依赖: rag_tokenizer(分词/词性/频率)、term_weight(词权重)、synonym(同义词)。

官方文件: rag/nlp/query.py@v0.24.0
"""
