...
"""文档二进制工具(与官方 rag/utils/file_utils.py 1:1 移植, 2026-09-02)。

按文件头魔数识别容器类型并对内嵌文件做"一层"解包, 以及从 docx/pdf 里抽取
超链接、抓取网页 HTML。naive.py 的内嵌文件提取与 hyperlink 分析依赖它。

- _guess_ext: 按 PK(OOXML/zip)/%PDF-/.doc 魔数猜测扩展名;
- extract_embed_file: 从 zip 容器(word/xl/ppt/embeddings) 或 OLE(doc) 里
  提取内嵌文件的 (文件名, bytes), 用内容 sha256 前 10 位去重 + 命名兜底;
- extract_links_from_docx / extract_links_from_pdf: 抽超链接;
- extract_html: 复用全局 session 带超时与重试抓网页原始字节。
"""
