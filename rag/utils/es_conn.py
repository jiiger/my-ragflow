...
"""ESConnection 落地类:混合检索的核心实现(DOC_ENGINE=elasticsearch 时装配)。

移植自官方 v0.24.0 rag/utils/es_conn.py(390 行, 逻辑 1:1)。
- search: 一次请求同时打 关键词 match + 稠密向量 knn + 稀疏向量 + 可选手工
  过滤, 再按 FusionExpr(RRF 等)融合排序——RAGFlow 混合检索的地基
- insert/update/delete: 带 knowledgebase_id 过滤条件与分页游标, 批量文档
  写入 chunk 索引
- 其余引擎(infinity/opensearch/oceanbase)官方同样式, 学习版只保留 ES"""
