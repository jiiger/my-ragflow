...
#
"""
rag.prompts.generator — 提示词运行时组装器(官方 932 行原样移植, 未做裁剪)。

模块顶部用 load_prompt 一次性加载 prompts/ 下全部 34 个 .md 模板为常量,
再把它们包成运行时函数, 按用途分族:
- 召回侧: keyword_extraction / question_proposal / full_question / cross_languages /
  multi_queries_gen / gen_meta_filter
- 生成侧: citation_prompt / citation_plus / question_prompt / content_tagging /
  vision_llm_*_prompt / structured_output_prompt
- 上下文拼装: kb_prompt / memory_prompt / chunks_format / form_message / form_history
- 文档结构(toc 目录感知): detect/extract/assign/transformer/run_toc_from_text 等一族
- Agent 编排: analyze_task_async / next_step_async / reflect_async / tool_call_summary /
  rank_memories_async / sufficiency_check
所有 LLM 调用都通过入参 chat_mdl(LLMBundle) 发起, 本模块不直接持有模型实例。
来源: 官方 rag/prompts/generator.py @ v0.24.0
"""
import asyncio
import datetime
import json
...
def kb_prompt(kbinfos, max_tokens, hash_id=False):
    ...
    from api.db.services.doc_metadata_service import DocMetadataService  # ⚠️ 元数据链未移植, 该函数运行时暂不可用
...
async def gen_json(system_prompt: str, user_prompt: str, chat_mdl, gen_conf={}, max_retry=2):
    from rag.graphrag.utils import get_llm_cache, set_llm_cache  # ⚠️ rag/graphrag 未移植, 该函数运行时暂不可用
