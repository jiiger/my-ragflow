"""rag/app/tag.py — 标签(Tag)文档解析器与问题分类, 对齐官方 v0.24.0(160 行)。

- chunk(): 解析"内容+标签"两列文件(Excel 或 csv/txt, 无表头), 产出带
  tag_kwd/tag_feas 的索引入库文档, 供知识库打标签使用
- label_question(): 租户知识库标签统计(all_tags_in_portion, 取 top ~15 个)
  + 问题与标签的检索命中数(tag_query) → 给问题打分类标签, 返回标签数组,
  供 Dealer.retrieval 的 rank_feature 当相关度特征(dialog/chunk 检索都用)
"""
