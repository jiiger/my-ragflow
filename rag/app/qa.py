"""rag/app/qa.py — 问答对(QA)文档解析器, 对齐官方 v0.24.0(482 行)。

chunk() 按扩展名分发:
- xlsx: Excel 子类(openpyxl 逐行取"问题-答案"两列, 变形行报失败数,
  random_choices 抽样 30 对判是否英文)
- txt/csv: 分隔符探测(TAB/comma), 单列行并入前一条答案或丢弃, 问答对成块
- md: mdQuestionLevel 按 # 层级组织问题栈, 答案经 markdown 表格渲染
- docx: Docx 子类(python-docx 段落遍历 + docx_question_level 题型层级 +
  段落图片 concat_img 拼接 + 表格转 HTML), 问题栈/答案栈收尾成块
- pdf: Pdf 子类 ⚠️ 依赖官方完整 PdfParser 的 OCR/版面分析管线(self.__images__/
  _layouts_rec/_table_transformer_job/_text_merge/_extract_table_figure/boxes/
  _line_tag/crop), 学习版 pdf_parser 未含 → 调用抛 AttributeError 属预期,
  移植完整 pdf_parser 后自动点亮
- beAdoc 系列: 问答对包装成索引文档(Question/Answer 前缀 + 分词字段 +
  可选图片/页码/行号)
"""
